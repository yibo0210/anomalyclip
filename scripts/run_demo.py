...
def test(args):
    ...
    model, _ = AnomalyCLIP_lib.load("ViT-B/16", device=device, design_details = AnomalyCLIP_parameters)
    model.eval()

    preprocess, target_transform = get_transform(args)

    # 自动适配 DPAM_layer：ViT-B/16 有 12 层，ViT-L/14 有 24 层
    num_layers = len(model.visual.transformer.resblocks)
    dpam_layer = min(20, num_layers + 1)

    prompt_learner = AnomalyCLIP_PromptLearner(model.to("cpu"), AnomalyCLIP_parameters)
    if args.checkpoint_path is not None and args.checkpoint_path.lower() != 'none':
...
if __name__ == '__main__':
    ...
    # paths
    parser.add_argument("--image_path", type=str, default="./assets/brain.png", help="path to test dataset")
