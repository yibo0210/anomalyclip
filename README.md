...
For users with limited GPU memory (e.g., 6GB), we recommend using the smaller `ViT-B/16` model instead of the default `ViT-L/14@336px`. You can specify the model via the `--model_name` argument:

```bash
python train.py --model_name ViT-B/16 --image_size 224 --batch_size 2
```

Make sure you have CUDA-enabled PyTorch installed. If you encounter CUDA errors, check your PyTorch installation with `torch.cuda.is_available()`.
