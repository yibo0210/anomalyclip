...
def convert_to_custom_text_state_dict(state_dict):
...
def resize_pos_embed(state_dict, model):
    if 'visual.positional_embedding' in state_dict and hasattr(model.visual, 'positional_embedding'):
        pos_embed = state_dict['visual.positional_embedding']
        model_pos_embed = model.visual.positional_embedding
        if pos_embed.shape != model_pos_embed.shape:
            old_size = int((pos_embed.shape[0] - 1) ** 0.5)
            new_size = int((model_pos_embed.shape[0] - 1) ** 0.5)
            if old_size != new_size:
                extra_tokens = pos_embed[:1, :]
                pos_tokens = pos_embed[1:, :]
                pos_tokens = pos_tokens.reshape(-1, old_size, old_size, pos_tokens.shape[-1]).permute(0, 3, 1, 2)
                pos_tokens = torch.nn.functional.interpolate(pos_tokens, (new_size, new_size), mode='bilinear')
                pos_tokens = pos_tokens.permute(0, 2, 3, 1).reshape(-1, pos_tokens.shape[1])
                pos_embed = torch.cat([extra_tokens, pos_tokens], dim=0)
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        # 优先从项目目录加载
        local_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "clip_models", name.replace("/", "-") + ".pt")
